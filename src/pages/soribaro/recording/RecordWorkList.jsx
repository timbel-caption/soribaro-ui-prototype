...
import { getRecordWorkRequests, getRecordWorkWorks } from '../../../api/v9';
...
export default function RecordWorkList({ mode, title, description }) {
  ...
  const REQUEST_SEARCH_FIELDS = useMemo(() => [
    { value: 'servCd', label: t('recording.searchFieldServiceCode') },
    { value: 'memberName', label: t('recording.searchFieldRequesterName') },
    { value: 'phone', label: t('recording.searchFieldContact') },
  ], [t]);

  const WORK_SEARCH_FIELDS = useMemo(() => [
    { value: 'servCd', label: t('recording.searchFieldServiceCode') },
    { value: 'title', label: t('recording.searchFieldTitle') },
    { value: 'memberName', label: t('recording.searchFieldRequesterName') },
    { value: 'phone', label: t('recording.searchFieldContact') },
    { value: 'workerName', label: t('recording.searchFieldWorkerName') },
  ], [t]);

  const SERV_TP_OPTIONS = useMemo(() => [
    { value: '1', label: t('recording.serviceTypeCallRecord') },
    { value: '2', label: t('recording.serviceTypeMultiRecord') },
  ], [t]);

  const CNL_YN_OPTIONS = useMemo(() => [
    { value: 'N', label: t('recording.cancelStatusNotCanceled') },
    { value: 'Y', label: t('recording.cancelStatusCanceled') },
  ], [t]);
  ...
  const CancelCellRenderer = useCallback((params) => {
    ...
    return <span className="cancel-badge cancel-yes">{t('recording.cancelStatusCanceled')}</span>;
  }, [t]);
  ...
  const defaultFilters = useMemo(() => {
    ...
    return {
      ...
      payTp: '',
      cnlYn: '',
      searchField: isRequest ? 'servCd' : 'servCd',
      searchText: '',
    };
  }, [isRequest]);
  ...
  const RequestStatusCellRenderer = useCallback((params) => {
    const label = params.data?.workStatNm || t('common.received');
    return <span className="status-badge status-1">{label}</span>;
  }, [t]);

  const requestColumnDefs = useMemo(() => [
    { field: 'servCd', headerName: t('recording.columnServiceCode'), width: 150 },
    { field: 'membNm', headerName: t('recording.columnRequester'), width: 100 },
    { field: 'mblTelNo', headerName: t('recording.columnContact'), width: 130 },
    { field: 'fileTp', headerName: t('recording.columnFileType'), width: 100, cellClass: 'text-center' },
    { field: 'payTp', headerName: t('recording.columnPaymentType'), width: 100, cellClass: 'text-center' },
    {
      field: 'fixPrice',
      headerName: t('recording.columnConfirmedPrice'),
      width: 110,
    ...
    {
      field: 'usePoint',
      headerName: t('recording.columnPoints'),
      width: 90,
      valueFormatter: (params) => formatAmount(params.value),
      cellClass: 'text-center',
    },
    { field: 'fileCnt', headerName: t('recording.columnFileCount'), width: 80, cellClass: 'text-center' },
    {
      field: 'regDttm',
      headerName: t('recording.columnRequestDate'),
      width: 140,
      valueFormatter: (params) => formatDateFull(params.value),
    },
    {
      field: 'workStatNm',
      headerName: t('recording.columnStatus'),
      width: 90,
    ...
    {
      field: 'cnlYn',
      headerName: t('recording.columnCancel'),
      width: 70,
      cellRenderer: CancelCellRenderer,
      cellClass: 'text-center',
    },
  ], [RequestStatusCellRenderer, CancelCellRenderer, t]);

  const workColumnDefs = useMemo(() => [
    { field: 'servCd', headerName: t('recording.columnServiceCode'), width: 150 },
    { field: 'servTitle', headerName: t('recording.columnTitle'), flex: 1, minWidth: 200, tooltipField: 'servTitle' },
    { field: 'membNm', headerName: t('recording.columnRequester'), width: 100 },
    { field: 'mblTelNo', headerName: t('recording.columnContact'), width: 130 },
    { field: 'fileTp', headerName: t('recording.columnFileType'), width: 100, cellClass: 'text-center' },
    {
      field: 'overallStatus',
      headerName: t('recording.columnWorkStatus'),
      width: 120,
    ...
    {
      field: 'worker',
      headerName: t('recording.columnWorker'),
      width: 140,
      tooltipValueGetter: (params) => params.data?.workerArr || '',
    },
    { field: 'totalPlayTm', headerName: t('recording.columnPlayTime'), width: 100, cellClass: 'text-center' },
    {
      field: 'fixPrice',
      headerName: t('recording.columnConfirmedPrice'),
      width: 110,
  ...
  const columnDefs = isRequest ? requestColumnDefs : workColumnDefs;
  const searchFields = isRequest ? REQUEST_SEARCH_FIELDS : WORK_SEARCH_FIELDS;
  ...
  const fetchData = useCallback(async (page = 0) => {
    ...
    try {
      const params = {
        ...
        ...(filters.servTp && { servTp: filters.servTp }),
        ...(filters.payTp && { payTp: filters.payTp }),
        ...(filters.searchText && { searchType: filters.searchField, searchText: filters.searchText }),
      };

      if (filters.cnlYn) params.cnlYn = filters.cnlYn;
      if (isRequest) {
        // 의뢰현황은 overallStatus 불필요
      } else {
        if (filters.overallStatus) params.overallStatus = filters.overallStatus;
      }

      const apiFn = isRequest ? getRecordWorkRequests : getRecordWorkWorks;
  ...
  }, [filters, pagination.size, isRequest]);

  const handleRowDoubleClicked = useCallback((event) => {
    const path = isRequest
      ? `/soribaro/recording/request/${event.data.servCd}`
      : `/soribaro/recording/work/${event.data.servCd}`;
    navigate(path);
  }, [navigate, isRequest]);
  ...
  return (
    <div className="notion-page record-work-page">
      ...
      <div className="filter-bar">
        ...
        <select
          className="filter-select"
          value={filters.servTp}
          onChange={(e) => handleFilterChange('servTp', e.target.value)}
        >
          <option value="">{t('recording.allServiceType')}</option>
          {SERV_TP_OPTIONS.map((opt) => (
        ...
        >
          <option value="">{t('recording.allCancelStatus')}</option>
          {CNL_YN_OPTIONS.map((opt) => (
            <option key={opt.value} value={opt.value}>{opt.label}</option>
          ))}
        </select>
