...
function MtgSettlementTab({ s }) {
  ...
  const [workers, setWorkers] = useState(() => {
    ...
    if (isRecordingSettle) {
      // 녹취록: 프로젝트 관리에 배정된 작업자를 기준으로 작업자별 정산 내역을 만든다.
      // 동일 작업자가 여러 프로젝트에 배정된 경우 각 프로젝트의 작업시간을 합산해 총 작업시간으로 반영하고, 등급은 Elite로 고정한다.
      const infoByWorker = new Map();
      subjects.forEach((proj) => {
        if (!proj.worker) return;
        const sec = (proj.projFiles || []).reduce((acc, f) => acc + durationToSec(f.workTime), 0);
        const prev = infoByWorker.get(proj.worker);
        if (prev) prev.sec += sec;
        else infoByWorker.set(proj.worker, { sec, accuracy: proj.accuracy || '-', errors: proj.errors ?? 0 });
      });
      return [...infoByWorker.entries()].map(([worker, info]) => ({
        worker,
        grade: 'Elite',
        workTime: secToDuration(info.sec),
...
function CompanySettlementTab({ s, isConfirmed, onConfirm, onReapply }) {
  ...
  const handleSaveFinal = () => {
    ...
    window.alert('최종 정산 금액이 저장되었습니다. 적용하려면 업체정산 확인을 눌러주세요.');
  };
    ...
    <div className="proto-tab-panel">
      <div className="proto-section-card">
        ...
        <div className="proto-section-card-body" style={{ padding: '16px 20px' }}>
          ...
          {noData && (
            <p style={{ fontSize: '13px', color: 'var(--text-secondary)', textAlign: 'center', margin: '12px 0 0' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '10px', marginTop: '16px', paddingTop: '14px', borderTop: '1px solid var(--border-color)' }}>
            ...
            <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
              ...
              {!showFinalSettlement && confirmButton}
            </div>
          </div>
        </div>
      </div>

      {/* 최종 정산 — 공급가액/세액을 수기로 입력하면 총합은 자동 계산된다 (업체 견적 세부 내용은 그대로 유지, 업체정산 확인을 눌러야 최종 확정됨) */}
      {showFinalSettlement && (
          ...
          <div className="proto-section-card-header">
            ...
            {savedFinal && (
              <span style={{ marginLeft: '8px', fontSize: '12px', color: '#22c55e', fontWeight: 600 }}>
                ✓ 저장됨 — 업체정산 확인을 눌러야 이 금액이 최종 확정됩니다
              </span>
            )}
          </div>
          <div className="proto-section-card-body" style={{ padding: '16px 20px' }}>
