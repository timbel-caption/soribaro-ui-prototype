...
export default function MeetingWorkPage() {
  ...
  const handleRegister = (form, files) => {
    ...
    appendMeetingSample({
      ...
      files: files.map((f, i) => ({ fileNo: i + 1, fileName: f.name, duration: form.fileDurations?.[i] || '-', size: f.size, uploadDttm: today })),
      assignments: [],
      manuals: [],
      workProgress: [],
      qcScore: null,
  ...
  return (
    <div className="notion-page enterprise-work-page">
      ...
      {showRegister && (
        <MeetingRegisterModal workType="meeting" onClose={() => { setShowRegister(false); refreshSamples(); }} onSubmit={handleRegister} />
      )}
    </div>
  );
}
